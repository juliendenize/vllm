...
def save_shards_if_rank0(model, dir):
    ...
    remapped_split = split_gate_up(state_dict)
    ...
    for name, tensor in remapped_final.items():
        ...
        if current_size + tensor_size > max_shard_size and current_shard:
            file_path = os.path.join(NEW_PATH, consolidated)
            save_file(current_shard, file_path)
            index_meta["weight_map"] = {key: consolidated for key in current_shard.keys()}
    ...
    index_path = os.path.join(NEW_PATH, "tmp.safetensors.index.json")
