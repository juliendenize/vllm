...
def split_expert_input_global_scales(state_dict):
    keys_to_process = [
        ...
        if ".experts." in k
        and (
            k.endswith("w13_input_global_scale") or k.endswith("w2_input_global_scale")
        )
    ]
    ...
    for key in keys_to_process:
        ...
        if key.endswith("w13_input_global_scale"):
            ...
            for i in range(num_experts):
                new_entries[f"layers.{layer_id}.experts.{i}.w1.input_global_scale"] = (
                    w1_vals[i].clone().reshape([1])
                )
                new_entries[f"layers.{layer_id}.experts.{i}.w3.input_global_scale"] = (
                    w3_vals[i].clone().reshape([1])
                )
            ...
            for i in range(num_experts):
                new_entries[f"layers.{layer_id}.experts.{i}.w2.input_global_scale"] = (
                    w2_vals[i].clone().reshape([1])
                )
...
def save_shards_if_rank0(model):
    ...
    state_dict = {
        key: torch.min(
            tensor_model_parallel_all_gather(value.unsqueeze(-1), dim=-1), dim=-1
        ).values
        for key, value in state_dict.items()
    }
    ...
    if current_shard:
        ...
        print(
            f"[Rank {rank}] Saved final shard {shard_index} ({len(current_shard)} tensors)."
        )

    # Write global index JSON
    index_path = os.path.join(PATH, "tmp.safetensors.index.json")
