...
def _validate_pixtral_pixel_values(
    pixel_values: object,
    *,
    ...
    for image_idx, image in enumerate(images):
        ...
        if image.shape[0] != expected_channels:
            raise ValueError(
                f"{field_name}[{image_idx}] must have {expected_channels} "
                f"channels, got {image.shape[0]}."
            )
    ...
    if source_images is not None:
        expected_count = (
            len(source_images) if isinstance(source_images, (list, tuple)) else 1
        )
    ...
    def get_hf_processor(self, **kwargs: object) -> ProcessorMixin:
        ...
        if is_mistral_tokenizer(tokenizer):
            ...
            if "size" in merged_kwargs:
                raise ValueError(
                    "Mistral tokenizer mode does not support `size` for Llava "
...
class PixtralHFMultiModalProcessor(BaseMultiModalProcessor[PixtralHFProcessingInfo]):
    ...
    def _postprocess_hf_mm_data(
        ...
        if isinstance(processor, MistralCommonPixtralProcessor):
            ...
            if images is None:
                raise ValueError(
                    "Llava native Pixtral processing is missing `images`; "
                    "the native processor must return image tensors."
                )
            processed_data["pixel_values"] = _validate_pixtral_pixel_values(
                images,
                expected_channels=3,
        ...
        def get_replacement(item_idx: int):
            ...
            if isinstance(processor, MistralCommonPixtralProcessor):
                try:
                    ...
                    raise ValueError(
                        "Llava native Pixtral processing cannot determine the "
                        "image patch grid from the native image processor."
                    ) from exc
            else:
